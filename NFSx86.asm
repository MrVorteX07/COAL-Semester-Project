...
intro_dev1 db 'Made with ', 03h, ' by:', 0
...
inst_8 db '= - Toggle Music in Menu', 0
inst_9 db 'M - Toggle Music in Game', 0
inst_10 db 'Press any key to continue...', 0
...
;---music-configuration-variables---
SPEAKER_PORT equ 61h
TIMER_PORT equ 42h
TIMER_CTRL equ 43h
PIT_FREQUENCY equ 1193180

music_enabled db 1          ;1 = music on, 0 = music off
current_music_track db 0    ;0=none, 1=menu, 2=gameplay, 3=gameover
music_position dw 0
music_tempo dw 1
music_speed dw 3
music_tempo_counter dw 0
music_paused db 0
channel_positions:  dw 0, 0, 0, 0
channel_durations:  dw 0, 0, 0, 0
channel_active:     db 1, 1, 1, 1
num_channels        db 4
current_channel_ptr dw 0
last_frequency dw 0
menu_music_channels: dw menu_music_ch0, menu_music_ch1, menu_music_ch2, menu_music_ch3
music_status_on db 'Music: ON ', 0
music_status_off db 'Music: OFF', 0

;sfx-flags
sfx_coin_trigger db 0
sfx_fuel_trigger db 0
sfx_crash_trigger db 0
sfx_pause_trigger db 0
sfx_resume_trigger db 0
...
;---music-tracks---
; Track format: [frequency_word, duration_word]
; Duration: 1=whole note, 2=half, 4=quarter, 8=eighth, etc.
; Ends with 0,0

;XM melody from 1221.xm
...
gameover_music:
    dw NOTE_C5, 4, NOTE_B4, 4, NOTE_A4, 4, NOTE_G4, 4
    dw NOTE_F4, 4, NOTE_E4, 4, NOTE_D4, 4, NOTE_C4, 2
    dw NOTE_REST, 2
    dw NOTE_C4, 8, NOTE_C4, 8, NOTE_C4, 4, NOTE_REST, 4
    dw 0, 0
...
;music-mute subroutine
check_and_toggle_music:
    push ax
    push bx
...
wait_key:
    ...
    ;set game as started
	mov word [music_enabled], 1
    mov byte [game_started], 1
	
    mov al, 2
    call change_music_track
...
set_easy_bg:
	mov word [music_speed], 6
    mov bl, 02h ;color
    jmp fill_screen_diff
    
set_medium_bg:
	mov word [music_speed], 3
    mov bl, 06h
    jmp fill_screen_diff
    
set_hard_bg:
	mov word [music_speed], 2
    mov bl, 04h
...
set_easy:
    ...
    mov word [fuel_decay_rate], 4 ; high = slow
    mov word [coin_value], 5  ; points
	mov word [music_speed], 8
    jmp done_apply
...
reset_trees_loop:
    ...
    mov byte [sfx_coin_trigger], 0
    mov byte [sfx_fuel_trigger], 0
    mov byte [sfx_crash_trigger], 0
    mov byte [sfx_pause_trigger], 0
    mov byte [sfx_resume_trigger], 0
    mov word [music_position], 0
    mov word [music_speed], 3
    mov word [sfx_position], 0
...
;keyboard-flag-checking subroutine
check_keyboard:
    push ax
...
check_movement:
    ; Check and handle all movement keys without clearing them yet
    mov al, [key_flag_left]
    cmp al, 1
    jne check_right
    mov byte [key_flag_left], 0  ; Clear BEFORE calling movement
...
check_right:
    ...
    cmp al, 1
    jne check_up
    mov byte [key_flag_right], 0  ; Clear BEFORE calling movement
...
move_player_left:
    ...
    ;dec and check whether collision
    dec al
    mov cl, al
...
show_pause_screen:
    ...
    mov al, [sfx_pause_trigger]
    cmp al, 1
    je skip_pause_sound
    
    mov byte [sfx_pause_trigger], 1
...
; New helper function - gets a random row, but ensures row 2 NPCs spawn far from player
get_safe_random_row:
    push bx
    call get_random
    mov bh, 10
    xor ah, ah
    div bh              ; ah = random row 0-9
    
    cmp ah, 2           ; is it player's row?
    jne safe_row_ok     ; if not, use it
    
    ; Row 2 - ensure safe distance (0 or 1 for far spawn)
    call get_random
    and al, 1           ; al = 0 or 1
    mov ah, al          ; put in ah (the row value)
    
safe_row_ok:
    mov al, ah          ; return row in al
    pop bx
    ret
...
play_note:
    ...
    mov ax, [music_tempo]
    mov cx, 800
    mul cx                  ;dx:ax = tempo * 1000
    div bx                  ;ax = delay in ms
...
init_music_system:
    ...
    mov byte [current_music_track], 0
    mov word [music_position], 0
...
;restore-music subroutine
restore_music_system:
    push ax
    push dx
    push ds
...
chain_timer:
    ...
    push word [cs:old_timer_seg]
    push word [cs:old_timer_off]
    retf
	
	
;process-sound-effects subroutine
process_sound_effects:
    ...
    mov al, [sfx_crash_trigger]
    cmp al, 1
    je start_crash_sfx
    
    mov al, [sfx_coin_trigger]
    cmp al, 1
    je start_coin_sfx
    
    mov al, [sfx_fuel_trigger]
    cmp al, 1
    je start_fuel_sfx
...
continue_sfx:
    dec word [sfx_duration]
    cmp word [sfx_duration], 0
    ja done_sfx_process
...
process_background_music:
	...
	mov bx, [music_speed]
    inc word [music_tempo_counter]
    mov ax, [music_tempo_counter]
    cmp ax, bx                   ;speed
    jb done_bg_music
    
    mov word [music_tempo_counter], 0
    
	mov al, [current_music_track]
    cmp al, 0
    je silence_music
    cmp al, 1
    je play_menu_track
    cmp al, 2
    je play_gameplay_track
    cmp al, 3
    je play_gameover_track
    jmp done_bg_music
    
play_menu_track:
    mov word [current_channel_ptr], menu_music_channels
    jmp process_channels
    
play_gameplay_track:
    mov si, gameplay_music
    jmp play_track
    
play_gameover_track:
    mov si, gameover_music

play_track:
    add si, [music_position]
    ...
    add word [music_position], 4
    jmp done_bg_music
    
restart_track:
    mov al, [current_music_track]
    cmp al, 3
    je silence_on_end            ;play once
	
    mov word [music_position], 0
    jmp done_bg_music

process_channels:
    xor cx, cx                  ;channel counter
    mov cl, [num_channels]
    xor di, di                  ;channel index
...
channel_loop:
	...
	;active check
    mov bx, di
    mov al, [channel_active + bx]
    cmp al, 0
    je next_channel
    
    ;duration check
    shl bx, 1
    mov ax, [channel_durations + bx]
    
    cmp ax, 0
    jne decrement_duration
    
    call load_next_note
    jmp next_channel
    
decrement_duration:
    dec word [channel_durations + bx]
...
load_next_note:
    ...
    ;get channel data pointer
    mov bx, di
    shl bx, 1
    mov si, [current_channel_ptr]
    
    ;get pointer to this channel data
    mov ax, di
    shl ax, 1
    add si, ax
    mov si, [si]                ;si = channel data
    
    mov bx, di
    shl bx, 1
    add si, [channel_positions + bx]
    
    mov ax, [si]                ;freq
    cmp ax, 0
    je end_of_channel
    
    mov dx, [si + 2]            ;duration
    
    ;prevent infinite notes
    cmp dx, 0
    je end_of_channel
    ...
    mov bx, di
    shl bx, 1
    mov [channel_durations + bx], dx
    
    ;move
    add word [channel_positions + bx], 4
...
end_of_channel:
    ...
    mov al, [current_music_track]
    cmp al, 3
    je deactivate_channel ;play once
    
    ;loop
    mov bx, di
    shl bx, 1
    mov word [channel_positions + bx], 0
    
    ;load a note when looping
    call do_reload
    jmp done_load
...
deactivate_channel:
    mov bx, di
    mov byte [channel_active + bx], 0
    
    ;clear duration for silence
    shl bx, 1
    mov word [channel_durations + bx], 0
...
check_active:
    ...
    ;channel 1 first (melody)
    mov di, 1
    call get_channel_note
    cmp ax, 0
    jne play_it
    ...
    ;channel 2 (percussion/pad)
    mov di, 2
    call get_channel_note
    cmp ax, 0
    jne play_it
    
    ;if no channel active play last freq
    mov ax, [last_frequency]
    cmp ax, 0
    je doneiip
...
;subroutine: get note from specific channel
;input: di = channel index
;output: ax = frequency (0 if none/rest)
get_channel_note:
    push bx
    push cx
    push dx
    ...
    ;check if channel has active duration
    shl bx, 1
    mov dx, [channel_durations + bx]
    cmp dx, 0
    je no_note
    ...
    shl bx, 1
    add si, [channel_positions + bx]
    sub si, 4                   ;go back to current note
...
reset_music_channels:
    ...
    xor ax, ax
    mov word [channel_positions], ax
    mov word [channel_positions + 2], ax
    mov word [channel_positions + 4], ax
    mov word [channel_positions + 6], ax
    
    mov word [channel_durations], ax
    mov word [channel_durations + 2], ax
    mov word [channel_durations + 4], ax
    mov word [channel_durations + 6], ax
    
    mov byte [channel_active], 1
    mov byte [channel_active + 1], 1
    mov byte [channel_active + 2], 1
    mov byte [channel_active + 3], 1
    
    mov word [music_tempo_counter], 0
    mov word [last_frequency], 0
...
play_frequency:
    ...
    ;convert frequency to PIT divisor
    mov bx, ax
...
; al = track number (1=menu, 2=gameplay, 3=gameover)
start_music_track:
    push ax
    call reset_music_channels
    pop ax
    mov [current_music_track], al
    ret

;change-music subroutine
change_music_track:
    push ax
...
;trigger-sound-effect subroutines
trigger_coin_sound:
    mov byte [sfx_coin_trigger], 1
    ret

trigger_fuel_sound:
    mov byte [sfx_fuel_trigger], 1
    ret

trigger_crash_sound:
    mov byte [sfx_crash_trigger], 1
    ret
...
trigger_resume_sound:
    mov byte [sfx_resume_trigger], 1
    ret

;toggle-music subroutine
toggle_music:
    push ax
...
draw_music_status:
    ...
    mov si, music_status_off
	mov bx, 1
    jmp draw_status
    
draw_on:
    mov si, music_status_on
